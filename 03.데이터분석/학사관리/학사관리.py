...
def pro_list():
    ...
    for idx, row in pro.iterrows():
        print(idx, row['교수이름'], row['교수학과'], row['임용일'], row['교수직급'], end=' ')
        print(f"{row['급여']:,}")

def pro_search(code):
    pro = pd.read_csv('학사관리/교수.csv', index_col='교수번호')
    stu = pd.read_csv('학사관리/학생.csv', index_col='학생번호')
    cou = pd.read_csv('학사관리/강좌.csv', index_col='강좌번호')
...
def stu_list():
    stu = pd.read_csv('학사관리/학생.csv')
    pro = pd.read_csv('학사관리/교수.csv')
    merge = stu.merge(pro, left_on='지도교수', right_on='교수번호')  #how = 'inner' 생략가능
    merge.sort_values('학생이름', inplace=True)
    for idx, row in merge.iterrows():
        print(idx+1, row['학생번호'], row['학생이름'], row['학생학과'], row['교수이름'], end=' ')
        print(row['생년월일'], row['학년'])

def stu_search(code):  #code = 학번
    stu = pd.read_csv('학사관리/학생.csv', index_col='학생번호')
    pro = pd.read_csv('학사관리/교수.csv', index_col='교수번호')
    enroll = pd.read_csv('학사관리/수강.csv', index_col=['학생번호', '강좌번호'])
...
def cou_list():
    cou = pd.read_csv('학사관리/강좌.csv')
    pro = pd.read_csv('학사관리/교수.csv')

    merge = cou.merge(pro, left_on='담당교수', right_on='교수번호')  #how = 'inner' 생략가능
    merge.sort_values('강좌이름', inplace=True)

    for idx, row in merge.iterrows():
        print(idx+1, row['강좌번호'], row['강좌이름'], row['강의실'], row['교수이름'], end=' ')
        print(row['강의시수'])

def cou_search(code):  #code = 강좌번호
    cou = pd.read_csv('학사관리/강좌.csv', index_col='강좌번호')
    pro = pd.read_csv('학사관리/교수.csv', index_col='교수번호')
    enroll = pd.read_csv('학사관리/수강.csv', index_col=['강좌번호', '학생번호'])
    enroll.fillna(0, inplace=True)
    stu = pd.read_csv('학사관리/학생.csv', index_col='학생번호')
